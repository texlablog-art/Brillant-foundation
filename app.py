...
@app.route('/admin/bulk-admit', methods=['GET'])
def bulk_admit():
    # ১. ফিল্টার ভ্যালু রিসিভ করা
    selected_class = request.args.get('class')
    selected_school = request.args.get('school')

    # ২. ফিল্টার অনুযায়ী কোয়েরি তৈরি
    query = {}
    if selected_class:
    ...
    # ৩. ডাটাবেস থেকে স্টুডেন্ট খুঁজে বের করা
    # শুধুমাত্র ভেরিফাইড স্টুডেন্টদের অ্যাডমিট জেনারেট করা নিরাপদ
    students = list(mongo.db.students.find(query))

    # ৪. স্কুল এবং ক্লাসের লিস্ট নিয়ে আসা (ফিল্টার ড্রপডাউনের জন্য)
    schools = mongo.db.students.distinct('institute_en')
    classes = ["5", "6", "7", "8", "9"]

    return render_template('admin_bulk_admit.html', 
                           students=students, 
                           schools=schools, 
                           classes=classes,
