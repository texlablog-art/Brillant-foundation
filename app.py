...
def export_detailed_data():
    ...
    # ৪. CSV জেনারেশন কনফিগারেশন
    output = io.StringIO()
    output.write(u'\ufeff') # Excel-এ বাংলা সাপোর্ট নিশ্চিত করার জন্য BOM
    ...
    # ৫. ডাটা প্রসেসিং লুপ
    for s in students:
        # --- নিরাপদভাবে সেন্টার তথ্য সংগ্রহ (KeyError সমাধান) ---
        c_code = s.get('center_code')
        center_info = mongo.db.centers.find_one({"center_code": c_code}) if c_code else None
        
        if center_info:
            # .get() ব্যবহার করা হয়েছে যাতে ডিকশনারিতে 'center_name_bn' না থাকলেও এরর না দেয়
            center_name = center_info.get('center_name_bn', c_code)
        else:
            center_name = c_code if c_code else "Unassigned"

        # --- নিরাপদভাবে মার্কস ডাটা সংগ্রহ ---
        marks = s.get('marks', {})
        # যদি ডাটাবেসে 'marks' ফিল্ডটি ভুল করে None রাখা হয়, তবে একে ডিকশনারি করে নেওয়া
        if not isinstance(marks, dict):
            marks = {}
        
        # বিষয়ভিত্তিক নম্বর (None হলে খালি স্ট্রিং দেখাবে)
        m_bangla = marks.get('bangla')
        m_english = marks.get('english')
        m_math = marks.get('math')
        m_gk = marks.get('gk')

        # টোটাল ক্যালকুলেশন (শুধুমাত্র নম্বরগুলো যোগ করা হবে, None বাদ দিয়ে)
        mark_list = [m_bangla, m_english, m_math, m_gk]
        valid_marks = [m for m in mark_list if isinstance(m, (int, float))]
        ...
        writer.writerow([
            ...
            total_marks if total_marks > 0 else ''
        ])

    # ৭. রেসপন্স তৈরি
    output.seek(0)
    # ফাইলের নামে ফিল্টার অনুযায়ী ক্লাস যোগ করা (ইউজার ফ্রেন্ডলি)
    cls_tag = selected_class if selected_class else "All"
    filename = f"TBF_Report_Class_{cls_tag}.csv"
...
@app.route('/admin/export-panel')
def export_panel():
    if not session.get('admin_logged_in'):
        return redirect(url_for('admin_login'))
    
    # ড্রপডাউনের জন্য ইউনিক স্কুলের লিস্ট নিয়ে আসা
    schools = mongo.db.students.distinct('institute_en')
    return render_template('export_filter.html', schools=schools)
